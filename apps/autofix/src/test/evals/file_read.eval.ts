...
eachModel('$modelName', ({ model }) => {
	describeEval('Get File Contents Tool', {
		data: async () => [
			{
				...
				expected: fmt.oneLine(`
					The getFileContents tool should be called with filePath="package.json"
			...
			},
			{
				...
				expected: fmt.oneLine(`
					The getFileContents tool should be called with filePath="README.md"
		...
		task: async (input: string) => {
			...
			const toolCall = toolCalls.find((call) => call.toolName === 'getFileContents')
			expect(toolCall, 'Tool getFileContents was not called').toBeDefined()
