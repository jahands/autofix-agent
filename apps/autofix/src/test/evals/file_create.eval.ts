...
eachModel('$modelName', ({ model }) => {
	describeEval('Create File Tool', {
		data: async () => [
			{
				...
				expected: fmt.oneLine(`
					The createFile tool should be called with filePath="hello.txt"
			...
			},
			{
				...
				expected: fmt.oneLine(`
					The createFile tool should be called with filePath="test.js"
		...
		task: async (input: string) => {
			...
			const toolCall = toolCalls.find((call) => call.toolName === 'createFile')
			expect(toolCall, 'Tool createFile was not called').toBeDefined()
