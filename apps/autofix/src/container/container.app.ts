...
import { execGetLinesStdoutStderr } from './execUtils.js'
import {
...
if (GIT_CLONE_URL) {
	for await (const { line } of execGetLinesStdoutStderr({
		cmd: `git clone ${GIT_CLONE_URL} repo`,
	})) {
		console.log(line)
	}
}
 ...
 */
app.post('/exec', zValidator('json', ExecParams), async (c) => {
	const lines: string[] = []
	try {
		...
		if (e instanceof Error) {
			return c.json({ error: e.message, lines }, 500)
		}
		return c.json({ error: 'Internal Error', lines }, 500)
	}
})
